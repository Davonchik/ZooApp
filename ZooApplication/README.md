...
| Класс                         | Назначение                                                                                      |
|-------------------------------|-------------------------------------------------------------------------------------------------|
| `CreateAnimalRequest`         | Данные для создания нового животного: имя, вид, дата рождения, пол, любимая еда, `EnclosureId`. |
...
| POST   | `/api/Animal/{id}/heal`     | Вылечить (поменять статус)                      |
| POST   | `/api/Animal/{id}/feed`     | Покормить животное                              |
...
| GET    | `/api/Enclosure/{id}`           | Информация по одному вольеру                        |
| DELETE | `/api/Enclosure/{id}`           | Удалить вольер                                      |
| PUT    | `/api/Enclosure/{id}`           | Обновить свойства вольера                           |
| POST   | `/api/Enclosure/{id}/clean`     | Провести уборку вольера                             |
...
| GET    | `/api/FeedingSchedule/{id}`           | Получить одну запись расписания                         |
| PUT    | `/api/FeedingSchedule/{id}`           | Обновить тип пищи и/или время кормления (замена записи) |
| PUT    | `/api/FeedingSchedule/{id}/reschedule`| Перепланировать время кормления                         |
| POST   | `/api/FeedingSchedule/{id}/complete`  | Отметить запись как выполненную                         |
| DELETE | `/api/FeedingSchedule/{id}`           | Удалить запись расписания                               |
...
| Method | URL                  | Description                                                                  |
|--------|----------------------|------------------------------------------------------------------------------|
| GET    | `/api/Statistics`    | Получить сводную статистику по зоопарку: общее количество животных, вольеров, свободных мест, ближайших кормлений |
...
2) Value Objects
   (Тут напиши все названия VO)
   Все проверяют свои инварианты в конструкторе (непустое имя, ненегативная вместимость, время в будущем и т.д.).
